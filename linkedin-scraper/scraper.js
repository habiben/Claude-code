...
async function searchPosts(keyword, creds) {
  ...
  const url = `/voyager/api/search/blended?keywords=${encoded}&origin=GLOBAL_SEARCH_HEADER&q=all&start=0&count=${MAX_POSTS}&filters=List(resultType-%3ECONTENT)`;
  const { status, body } = await liGet(url, creds);
  if (status !== 200 || typeof body !== 'object') return [];
  const urns = [];
  for (const group of (body?.elements || body?.data?.elements || [])) {
    for (const item of (group.elements || [])) {
      const urn = item.targetUrn || item.entityUrn || '';
      if (urn.includes('activity') || urn.includes('ugcPost') || urn.includes('share')) urns.push(urn);
    }
  }
  return urns;
}
...
async function fetchPost(urn, creds) {
  const url = `/voyager/api/feed/updates/${encodeURIComponent(urn)}?updateType=STORY_UPDATE&ursaContextType=FEED_DETAIL`;
  const { status, body } = await liGet(url, creds);
  ...
  for (const keyword of keywords) {
    ...
    try {
      ...
      for (const urn of fresh) {
        ...
        if (!post) continue;
        post.commentsList = post.comments > 0 ? await (async () => { await sleep(500); return fetchComments(urn, creds); })() : [];
